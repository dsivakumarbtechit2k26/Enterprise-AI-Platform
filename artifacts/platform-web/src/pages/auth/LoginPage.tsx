...
/** Navigate browser to the OAuth redirect endpoint — backend performs the actual redirect. */
...
export default function LoginPage() {
  ...
  // Fetch public settings to determine which OAuth providers are enabled.
  // Stale-while-revalidate with a 5-minute cache — no need for real-time precision.
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
      <div className="w-full max-w-md bg-card border shadow-sm rounded-xl p-8 space-y-6">
        ...
        {/* OAuth section — only rendered when at least one provider is enabled */}
        {showOAuthSection && (
          <>
            ...
            <div className={`grid gap-4 ${githubEnabled && googleEnabled ? "grid-cols-2" : "grid-cols-1"}`}>
              {githubEnabled && (
                <Button
                  variant="outline"
                  type="button"
